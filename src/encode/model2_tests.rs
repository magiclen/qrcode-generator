...
fn ki_001_annex_i_format_value_matches() {
    assert_eq!(format_bits(QrErrorCorrection::Medium, 2), 0x5E7C);
}
...
fn ki_005_n4_zero_band_endpoints_are_inclusive() {
    assert_eq!(n4_penalty(180, 400), 0);
    assert_eq!(n4_penalty(200, 400), 0);
    assert_eq!(n4_penalty(220, 400), 0);
}
...
fn ki_005_n4_scores_one_step_just_outside_the_zero_band() {
    assert_eq!(n4_penalty(179, 400), PENALTY_N4);
    assert_eq!(n4_penalty(221, 400), PENALTY_N4);
}
...
fn ki_005_n4_steps_grow_by_five_percent_bands() {
    assert_eq!(n4_penalty(160, 400), PENALTY_N4);
    assert_eq!(n4_penalty(159, 400), 2 * PENALTY_N4);
    assert_eq!(n4_penalty(0, 400), 9 * PENALTY_N4);
    assert_eq!(n4_penalty(400, 400), 9 * PENALTY_N4);
}
