...
#[cfg(feature = "validator")]
use validators::{http_ftp_url::HttpFtpUrlValidator, http_url::HttpUrlValidator, ValidatorOption};
...
fn text_optimize_with_validated_http_url_segments() {
    ...
    let matrix_2 = qrcode_generator::to_matrix_by_segments(
        &qrcode_generator::optimize_validated_http_url_segments(
            &validated_http_url,
            QrCodeEcc::Low,
        )
        .unwrap(),
...
fn text_optimize_with_validated_http_ftp_url_segments() {
    ...
    let matrix_1 = qrcode_generator::to_matrix(url, QrCodeEcc::Low).unwrap();
    let matrix_2 = qrcode_generator::to_matrix_by_segments(
        &qrcode_generator::optimize_validated_http_ftp_url_segments(
            &validated_http_ftp_url,
            QrCodeEcc::Low,
        )
        .unwrap(),
        QrCodeEcc::Low,
...
fn text_to_svg_to_file() {
    qrcode_generator::to_svg_to_file(
        ...
        256,
        Some(""),
        Path::join(Path::new(FOLDER), "hello_output.svg"),
    )
    .unwrap();

    assert_eq!(
        fs::read(Path::join(Path::new(FOLDER), "hello.svg")).unwrap(),
        fs::read(Path::join(Path::new(FOLDER), "hello_output.svg")).unwrap()
    );
}
...
fn text_to_png_to_file() {
    qrcode_generator::to_png_to_file(
        ...
        QrCodeEcc::Low,
        256,
        Path::join(Path::new(FOLDER), "hello_output.png"),
    )
    .unwrap();

    assert_eq!(
        fs::read(Path::join(Path::new(FOLDER), "hello.png")).unwrap(),
        fs::read(Path::join(Path::new(FOLDER), "hello_output.png")).unwrap()
    );
}
